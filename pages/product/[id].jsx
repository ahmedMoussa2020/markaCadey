...
const Product = ({ kala }) => {
  const [price, setPrice] = useState(kala.prices[0]);
  ...
  const handleSize = (sizeIndex) => {
    const difference = kala.prices[sizeIndex] - kala.prices[size];
    setSize(sizeIndex);
  ...
  const handleClick = () => {
    dispatch(addProduct({ ...kala, extras, price, quantity }));
  };
        ...
        <div className={styles.ingredients}>
          {kala.extraOptions.map((option) => (
            <div className={styles.option} key={option._id}>
              ...
              ></input>
              <label htmlFor="Mild">{option.text}</label>
            </div>
          ))}
        </div>
